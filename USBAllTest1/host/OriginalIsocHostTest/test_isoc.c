...
int main(int argc, char **argv)
{
	...
	memset(transfer_in_array, 0, sizeof(transfer_in_array));
	memset(transfer_out_array, 0, sizeof(transfer_out_array));

	for(int i = 0; i < MAX_ISOC_IN_TRANSFERS; i++ ) {
		transfer_in_array[i].is_pending_flag = 0;
		transfer_in_array[i].transfer_buff = libusb_alloc_transfer(NUM_ISO_PACKETS_TO_TRANSFER);
	}

	for(int i = 0; i < MAX_ISOC_OUT_TRANSFERS; i++ ) {
		transfer_out_array[i].is_pending_flag = 0;
		transfer_out_array[i].transfer_buff = libusb_alloc_transfer(NUM_ISO_PACKETS_TO_TRANSFER);
	}
	...
	for (;1;){//int loopLimit = 0; loopLimit < 10000; loopLimit++) {
	    for(int i = 0; i < MAX_ISOC_IN_TRANSFERS; i++ ) {
		    if( transfer_in_array[i].is_pending_flag ) {
			    continue;
		    }

		    printf("Submitting isoc READ transfer request\n");
		    memset(transfer_in_array[i].isoc_buffer, 0, sizeof(transfer_in_array[i].isoc_buffer));

		    libusb_fill_iso_transfer(
				    transfer_in_array[i].transfer_buff, devh, ISOC_IN_EP, transfer_in_array[i].isoc_buffer,
				    MAX_ISOC_PACKET_SIZE, NUM_ISO_PACKETS_TO_TRANSFER, isoc_input_completion_handler,
				    &transfer_in_array[i], 1000);

		    libusb_set_iso_packet_lengths(transfer_in_array[i].transfer_buff, MAX_ISOC_PACKET_SIZE);
		    transfer_in_array[i].is_pending_flag = 1;

		    r = libusb_submit_transfer(transfer_in_array[i].transfer_buff);
            if (r != 0){
                printf("Submit isoc READ FAILED: %i\n",r);
            }           
	    }
        for(int i = 0; i < MAX_ISOC_OUT_TRANSFERS; i++ ) {
            if (transfer_out_array[i].is_pending_flag){
                continue;
            }
            if ((turnstyle++ % 1000000) == 0){
                printf("Submitting isoc WRITE transfer request\n");
                libusb_fill_iso_transfer(
                        transfer_out_array[i].transfer_buff, devh, ISOC_OUT_EP, transfer_out_array[i].isoc_buffer,
                        MAX_ISOC_PACKET_SIZE, NUM_ISO_PACKETS_TO_TRANSFER, isoc_output_transfer_completion_handler,
                        &transfer_out_array[i], 1000);

                libusb_set_iso_packet_lengths(transfer_out_array[i].transfer_buff, MAX_ISOC_PACKET_SIZE);
                transfer_out_array[i].transfer_buff->iso_packet_desc[i].actual_length = 1;
                transfer_out_array[i].transfer_buff->iso_packet_desc[i].length = MAX_ISOC_PACKET_SIZE;

                transfer_out_array[i].isoc_buffer[0] ^= 1;

                r= libusb_submit_transfer(transfer_out_array[i].transfer_buff);

                if (r != 0){
                    printf("Submit isoc WRITE FAILED: %i\n",r);
                }else{
                    transfer_out_array[i].is_pending_flag=1;
                }
            } 
        }       
    }
